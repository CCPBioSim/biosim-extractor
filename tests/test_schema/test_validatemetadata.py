...
def test_validate_metadata_strict(monkeypatch):
    ...
    with pytest.raises(ValueError):
        vs.validate_metadata({'foo': 'bar'}, biosimschema_path="dummy.yaml", strict=True)

def test_validate_metadata_warning(monkeypatch):
    monkeypatch.setattr(vs, "validate_extracted", lambda *a, **kw: ["err1"])
    with warnings.catch_warnings(record=True) as w:
        warnings.simplefilter("always")
        vs.validate_metadata({'foo': 'bar'}, biosimschema_path="dummy.yaml", strict=False)
...
def test_validate_extracted_matrix_and_vector(monkeypatch):
    ...
    class DummyReport:
        results = [types.SimpleNamespace(message="errA"), types.SimpleNamespace(message="errB")]
