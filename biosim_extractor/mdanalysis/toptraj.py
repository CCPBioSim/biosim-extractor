...
def get_protein_sequence(fragment):
    ...
    try:
        protein_atoms = fragment.select_atoms("protein and not (resname ACE or resname NME)")
...
def classify_box(dim, tolerance=1e-3):
    ...
    def close(x, y): return abs(x - y) < tolerance
...
TOPTRAJ_AUTO_EXTRACT = {
    ...
    "box_dimensions": lambda u: safe_extract(lambda: np.mean([f.dimensions[:3] for f in u.trajectory], axis=0)),
    "box_angles": lambda u: safe_extract(lambda: list(u.trajectory[0].dimensions[3:].flatten())),
    ...
    "positions": lambda u: safe_extract(lambda: getattr(u.trajectory.ts, "positions", None) is not None),
    "forces": lambda u: safe_extract(lambda: getattr(u.trajectory.ts, "forces", None) is not None),
    "velocities": lambda u: safe_extract(lambda: getattr(u.trajectory.ts, "velocities", None) is not None),
    "masses": lambda u: safe_extract(lambda: getattr(u.atoms, "masses", None) is not None),
    "bonds": lambda u: safe_extract(lambda: getattr(u, "bonds", None) is not None),
    "dihedrals": lambda u: safe_extract(lambda: getattr(u, "dihedrals", None) is not None),
    "fixed_charges": lambda u: safe_extract(lambda: getattr(u.atoms, "charges", None) is not None),
...
MOLID_AUTO_EXTRACT = {
    ...
    "molecule_charge": lambda fragment: safe_extract(lambda: sum(list(fragment.atoms.charges))),
    'molecular_weight': lambda fragment: float(sum(fragment.masses)) if hasattr(fragment, 'masses') else None,
}
...
RDKIT_AUTO_EXTRACT = {
    ...
    "molecular_formula": lambda rdkit_mol: Chem.rdMolDescriptors.CalcMolFormula(rdkit_mol),
    "molecular_weight": lambda rdkit_mol: Chem.rdMolDescriptors.CalcExactMolWt(rdkit_mol),
}
    ...
    def _find_molecule_IDs(self):
        ...
        for i, (signature, info) in enumerate(self.molecule_types.items()):
            ...
            if len(mol_atoms) == 1:
                ...
                molecule_ids[i]['molecular_formula'] = getattr(atom, 'element', atom.name.strip('0123456789'))
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description="Extract topology and trajectory files metadata to JSON")
